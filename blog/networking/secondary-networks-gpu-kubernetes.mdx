...
*For a hands-on walkthrough of these network types with working manifests, see our [GH200 dual-network RDMA guide](/blog/dual-network-rdma-kubernetes-gh200) (macvlan).*

<ServiceCTA
    service="GPU Networking"
    title="Picking the wrong secondary network costs you GPU hours"
    text="Whether it's macvlan for a POC, host-device for production single-tenant training, or SR-IOV for multi-tenant clusters - we help GPU teams choose, deploy, and debug the Network Operator stack end to end."
    link="/services/gpu-networking"
    buttonLabel="Get RDMA networking help"
/>
